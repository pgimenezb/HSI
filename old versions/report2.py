...
# === Generador de mezclas sintéticas (solo 2 pigmentos) ===
def generate_synthetic_mixtures(R_pigments, n_samples=6000):
    R_pigments = np.array(R_pigments)
    if R_pigments.ndim > 2:
        R_pigments = R_pigments.reshape(R_pigments.shape[0], -1)
    ...
    for _ in range(n_samples):
        # Elegir 2 pigmentos distintos
        idx = np.random.choice(n_pigments, 2, replace=False)

        # Proporciones aleatorias con un sesgo leve (20–80% o 50–50%)
