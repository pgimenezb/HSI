...
# Decoders (soft)
# ─────────────────────────────────────────────────────────────────────────────
def decode_soft_pigment_and_group(y_like: np.ndarray, n_p: int, threshold=0.5):
    y_pig = y_like[:, :n_p]
...
def decode_soft_pigment_and_mix4(y_like: np.ndarray, n_p: int, threshold=0.5):
    y_pig = y_like[:, :n_p]
    y_mix = y_like[:, n_p:n_p+4]
...
def export_splits_csv(
    ...
    def save_subset(indices: np.ndarray, name: str):
        ...
        paths[name] = path
        print(f"[SAVE] Split CSV ({name}) -> {path}")
...
def main():
    ...
    for name in model_names:
        ...
        # === Carpetas ===
        base_out = os.path.join(out_dir, f"{name}_coactivation")
        ...
        def save_cm(cm, classes, title, fname, label):
            """Guarda una matriz de confusión y devuelve la ruta guardada."""
        ...
        # 1️⃣ Pigments
        cm_pig_soft = soft_confusion_matrix(y_test[:, :n_p], y_pred_prob[:, :n_p], classes_pig)
        path_pig = save_cm(cm_pig_soft, classes_pig, "Pigments", f"{name}_cm_PIGMENTS.png", "PIGMENTS")

        # 2️⃣ Mixtures
        cm_mix_soft = soft_confusion_matrix(y_test[:, n_p+1:], y_pred_prob[:, n_p+1:], ["M1", "M2", "M3"])
        path_mix = save_cm(cm_mix_soft, ["M1", "M2", "M3"], "Mixtures", f"{name}_cm_MIXTURES.png", "MIXTURES")

        # 3️⃣ PURE vs MIXTURE (soft, por pigmento)
        ...
        # Agrupar PURE vs (M1, M2, M3)
        y_true_puremix = np.stack([
            y_true_pigments * y_true_mixtures[:, [0]],  # Pure
            y_true_pigments * y_true_mixtures[:, 1:].sum(axis=1, keepdims=True)  # Mixture
        ], axis=2).reshape(len(y_test), -1)

        y_pred_puremix = np.stack([
            y_pred_pigments * y_pred_mixtures[:, [0]],
            y_pred_pigments * y_pred_mixtures[:, 1:].sum(axis=1, keepdims=True)
        ], axis=2).reshape(len(y_test), -1)

        # ✅ Orden alternado por pigmento
        classes_puremix_soft = [lab for i in range(n_p) for lab in (f"P{i+1:02d}_Pure", f"P{i+1:02d}_Mixture")]

        cm_puremix_soft = soft_confusion_matrix(
            y_true_puremix,
            y_pred_puremix,
            classes_puremix_soft,
            normalize="row"
        )

        path_puremix = save_cm(
            cm_puremix_soft,
            classes_puremix_soft,
            "Pure vs Mixture",
            f"{name}_cm_PURE_vs_MIXTURE.png",
            "PURE vs MIXTURE"
        )

        # 4️⃣ SOFT perPigmentMix (expandida Pigmento × Mezcla)
        classes_soft = [f"P{i+1:02d}_{mix}" for i in range(n_p) for mix in ["Pure", "M1", "M2", "M3"]]

        # Expandir combinaciones pigmento × mezcla (producto cartesiano)
        y_true_expanded, y_pred_expanded = [], []
        for i in range(n_p):
            for j in range(4):
                y_true_expanded.append((y_true_pigments[:, i] * y_true_mixtures[:, j])[:, np.newaxis])
                y_pred_expanded.append((y_pred_pigments[:, i] * y_pred_mixtures[:, j])[:, np.newaxis])

        y_true_soft = np.concatenate(y_true_expanded, axis=1)
        y_pred_soft = np.concatenate(y_pred_expanded, axis=1)

        cm_soft_perPigMix = soft_confusion_matrix(y_true_soft, y_pred_soft, class_names=classes_soft, normalize="row")
        path_soft = save_cm(cm_soft_perPigMix, classes_soft, "Pigments × Mixtures (Soft multi-label)", f"{name}_cm_SOFT_perPigmentMix.png", "SOFT perPigmentMix")
