...
def build_cnn(input_len, num_classes, trial):
    ...
    m.add(MaxPooling1D(2)); m.add(BatchNormalization()); m.add(Dropout(trial.suggest_float("dropout_1",0.3,0.6)))
    # Bloque 2
    ...
    m.add(MaxPooling1D(2)); m.add(BatchNormalization()); m.add(Dropout(trial.suggest_float("dropout_2",0.3,0.6)))
    # Cabeza
...
def tune_and_train(X_train,y_train,X_val,y_val,input_len,num_classes, trials=30,epochs=50,
    ...
    batch_choices=kw.get("batch_choices",[32,64,128]); patience=int(kw.get("patience",5))
    if storage is None: storage=f"sqlite:///{os.path.abspath('outputs/optuna.db')}"
    if study_name is None: study_name=f"cnn_baseline_{time.strftime('%Y%m%d')}"
    ...
    best=study.best_trial.params; model=build_cnn(input_len,num_classes,optuna.trial.FixedTrial(best))
