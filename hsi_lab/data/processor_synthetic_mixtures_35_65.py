...
def generate_synthetic_2pigment_mixtures(
    ...
    for i, (p1, p2) in enumerate(full_pairs):
        ...
        # Random dominant weight (balanced around 50%)
        w1 = np.clip(np.random.normal(0.5, 0.15), 0.35, 0.65)
        w2 = 1.0 - w1
    ...
    # === Save CSV (only once) ===
    mix_info_path = os.path.join(out_dir, "processor_synthetic_mixtures_35_65.csv")
    ...
    freq_path = os.path.join(out_dir, "processor_synthetic_mixtures_frequency_35_65.csv")
    pd.DataFrame({
...
cd ~/projects/HSI
python -m hsi_lab.data.processor_synthetic_mixtures_35_65
"""