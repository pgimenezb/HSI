...
# ======================================================================
# GENERATE SYNTHETIC 2-PIGMENT MIXTURES (80:20)
# ======================================================================
def generate_synthetic_2pigment_mixtures(
    ...
    print("\n[PHASE A] Generating and evaluating synthetic 2-pigment mixtures with 80:20 distribution...")
    ...
    all_pairs = [(i, j) for i in range(n_pigments) for j in range(n_pigments) if i != j]
    random.shuffle(all_pairs)

    total_pairs = len(all_pairs)
    reps_per_pair = max(1, n_samples // total_pairs)
    print(f"[DEBUG] {total_pairs} possible pigment pairs; each repeated {reps_per_pair}× on average.")

    # --- Step 2: expand pairs to reach target n_samples ---
    full_pairs = []
    ...
    # === Save CSV (only once) ===
    mix_info_path = os.path.join(out_dir, "processor_synthetic_mixtures_80_20.csv")
    ...
    freq_path = os.path.join(out_dir, "processor_synthetic_mixtures_frequency_80_20.csv")
    pd.DataFrame({
...
if __name__ == "__main__":
    ...
    print("[OK] DataFrame loaded — generating synthetic mixtures.")
...
cd ~/projects/HSI
python -m hsi_lab.data.processor_synthetic_mixtures_80_20
"""
