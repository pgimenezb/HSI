...
class HSIDataProcessor:
    ...
    def dataframe(self, mode: str = "filtered", debug_csv_dir: str | None = None):
        ...
        if RAW:
            # Aun en modo RAW, eliminamos las primeras bandas SWIR (swir_1 → swir_35)
            cols_to_drop = [f"swir_{i}" for i in range(1, 36)]
            df_merged = df_merged.drop(columns=[c for c in cols_to_drop if c in df_merged.columns], errors="ignore")

            # Debug 
            vis_cols = [c for c in df_merged.columns if c.startswith("vis_")]
            swir_cols = [c for c in df_merged.columns if c.startswith("swir_")]
            print(f"[DEBUG][RAW MODE] VIS: {len(vis_cols)}, SWIR: {len(swir_cols)}, TOTAL: {len(vis_cols) + len(swir_cols)}")
        ...
        # === Eliminar las primeras 35 bandas SWIR (swir_1 → swir_35) ===
        cols_to_drop = [f"swir_{i}" for i in range(1, 36)]
        df_out = df_out.drop(columns=[c for c in cols_to_drop if c in df_out.columns], errors="ignore")
        df_out = df_out.loc[:, ~df_out.columns.duplicated()]
         
        # === Eliminar filas que no son "Pure" en columna "Mixture" ===
        df_out = df_out[df_out["Mixture"] == "Pure"]

        
        # === Eliminar las 5 últimas posiciones del array en la columna "Multi" ===
        if "Multi" in df_out.columns:
            n_pigments = int(self.variables.get("num_files", 0))
            print(f"[DEBUG] Keeping only the first {n_pigments} positions of 'Multi' (num_files from config).")

            df_out["Multi"] = df_out["Multi"].apply(
                lambda x: np.array(x)[:n_pigments] if isinstance(x, (list, np.ndarray)) else x
            )
        df_out = df_out.loc[:, ~df_out.columns.duplicated()]
...
if __name__ == "__main__":
    print("\n[INIT] Loading and building dataframe from HSI files...")
    
    # Instancia el procesador con las variables del proyecto
    processor = HSIDataProcessor(variables)
    
    # Carga los archivos HDF5
    processor.load_h5_files()
    
    # Genera el dataframe (modo filtrado por defecto)
    df_used = processor.dataframe(mode="filtered")
    
    # Guarda el resultado en CSV dentro de la carpeta data/
    out_csv = "/home/pgimenez/projects/HSI/hsi_lab/data/processor_pure_pigments.csv"
    df_used.to_csv(out_csv, index=False)
    
    print(f"[SAVE] DataFrame CSV -> {out_csv}")
    print(f"[INFO] Shape: {df_used.shape}")
    print(f"[INFO] Columns: {len(df_used.columns)} columns")


    """
    To execute on the terminal: 
    cd ~/projects/HSI:
    python -m hsi_lab.data.processor_pure_pigments
    """